 ...
 * @author Admin
 */
public class ClassicAlgorithm implements IArgorithm{
    ...
    // Kiểm tra xem 2 điểm có thể nối theo hình chữ Z hay chữ L không, xuất phát theo chiều ngang trước
    private boolean checkZLX (Board board, Cell c1, Cell c2) {
        if (c1.getX() == c2.getX()) {
            return false;
        }
        for (int i = c1.getY(); i <= c2.getY(); i++) {
            ...
            if (checkLineY(board, board.getCell(c1.getX(), i), c2) == true) {
                return true;
            }
            if (checkLineY(board, board.getCell(c1.getX(), i), board.getCell(c2.getX(), i)) && checkLineX(board, board.getCell(c2.getX(), i), c2)) {
                return true;
            }
        }
        return false;
    ...
    //  Kiểm tra xem 2 điểm có thể nối theo hình chữ Z hay chữ L không, xuất phát theo chiều dọc trước
    private boolean checkZLY (Board board, Cell c1, Cell c2) {
        if (c1.getY() == c2.getY()) {
            return false;
        }
        for (int i = c1.getX(); i <= c2.getX(); i++) {
            ...
            if (checkLineX(board, board.getCell(i, c1.getY()), c2) == true) {
                return true;
            }
            if (checkLineX(board, board.getCell(i, c1.getY()), board.getCell(i, c2.getY())) && checkLineY(board, board.getCell(i, c2.getY()), c2)) {
                return true;
            }
        }
        return false;
    ...
    public boolean checkPath (Board board, Cell c1, Cell c2) {
        ...
        if (checkZLX(board, c1, c2)) {
            return true;
        }
        if (checkZLY(board, c1, c2)) {
            return true;
        }
        return false;
    }
    ...
    public boolean hasAnyMatch (Board board) {
        for (Integer key : board.getMap().keySet()) {
            int size = board.getMap().get(key).size();
            for (int i = 0; i < size; i++) {
                if (board.getMap().get(key).get(i).isStatus() == true) {
                    for (int j = i + 1; j < size; j++) {
                        if (board.getMap().get(key).get(j).isStatus() == true) {
                            if (checkPath(board, board.getMap().get(key).get(i), board.getMap().get(key).get(j))) {
                                return true;
                            }
                        }
                    }
